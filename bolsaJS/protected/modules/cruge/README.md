...
##Manejando los usuarios con el API de Cruge:

	Yii::app()->user->um te da acceso al API de usuarios de Cruge (cruge\components\CrugeUserManager.php)
	...
	<?php
		//  LEYENDO UN CAMPO PERSONALIZADO DEL USUARIO QUE HIZO LOGIN.
		//		Esta es una forma mas corta que la tradicional la cual sería:
	...
	<?php
		// ARMANDO UN COMBOBOX CON LA LISTA DE USUARIOS REGISTRADOS MOSTRANDO:
...
Las variables del sistema se pueden modificar mediante el menu de administracion principal, en el item 'Variables del Sistema',
las variables controlan el comportamiento de Cruge, por ejemplo, la pantalla de Registro, detener el sistema, duracion de la sesión, etc.

Tambien puedes acceder a las variables del sistema mediante API:

		<?php
			// ASI SE CONSULTA UNA VARIABLE DE SISTEMA. ESTAS SE ENCUENTARN LISTADAS EN: '/data/CrugeSystem.php'
			//
...
El menu 'Variables del Sistema' es aquel que aparece dentro del submenu que aparecera cuando accedes a esta URL:
